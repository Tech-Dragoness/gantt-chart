...
def save_to_github(data, sha=None):
    ...
    payload = {
        "message": f"Update gantt data - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
...
def update_data():
    ...
    try:
        ...
        print(f"Received data with {len(data)} tasks")
...
def backup_data():
    ...
    return jsonify(data), 200, {
        'Content-Disposition': 'attachment; filename=gantt_backup.json'
    }
